...
```{r}
modelSvm <- svm(salePrice~., data = train_set, scale = F)
summary(modelSvm)
modelSvm$index
plot(modelSvm, train_set, salePrice~.)
```
...
``` {r}
modelSVM_L<-svm(salePrice~., data=train_set, cost=2^5, kernel="linear") #98%
modelSVM_L<-svm(salePrice~., data=train_set, cost=2^-5, kernel="linear") #88%
modelSVM_L<-svm(salePrice~., data=train_set, cost=0.5, kernel="linear")#95%
modelSVM_R<-svm(salePrice~., data=train_set, gamma=2^-5, kernel="radial")
modelSVM_R<-svm(salePrice~., data=train_set, gamma=2^1, kernel="radial")
```

### --> Predicciones

``` {r}
predictionL<-predict(modelSVM_L, newdata=train_set[,-21])
predictionR<-predict(modelSVM_R, newdata=train_set[,-21])
```


### --> Tunear modelo y obtener una mejor prediccion
``` {r}
modelTuned <- tune.svm(salePrice~., data=train_set, cost=c(0.01,0.1,0.5,1,5,10,16,20,32), kernel="linear")
predBestModelo <- predict(modelTuned$best.model,newdata = test_set[,-24])
```
