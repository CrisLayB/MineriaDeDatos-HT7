...
suppressWarnings({
  ...
  modelTunedP_prof_ej8 <- tune.svm(economy~., data=train_set, cost=c(0.01,0.1,0.5,1,5,10,16,20,32), kernel="polynomial", degree = c(2,3))
  Rprof(NULL)
  profile_polynomial_model_ej8 <- summaryRprof(memory = "both")
})
```
...
tiempo:`r profile_nb_model$sampling.time` 
uso de memoria: `r sum(profile_polynomial_model_ej8$by.total$mem.total)` -> la unidad está en megas.
